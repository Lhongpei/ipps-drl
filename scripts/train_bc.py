...
from ipps_drl.utils.utils import solutions_padding
from ipps_drl.validate import validate, get_validate_env
from ipps_drl.models.bc import BehaviorCloning
from ipps_drl.dataset import ILDataScheduler, collate_fn
from ipps_drl.models.memory import MemoryIL
...
if __name__ == '__main__':
    ...
    dir_dict = ".cache/il_data/{0}{1}/".format(str.zfill(str(env_paras["num_jobs"]),2),
                                                   str.zfill(str(env_paras["num_mas"]),2))

    dir_dict = ".cache/il_data/0605/"
    scheduler = ILDataScheduler(config, dir_dict, device)
    dataset_loader = scheduler.load_dataset(shuffle=False)
    validate_path = "data/dev/{0}{1}/".format(str.zfill(str(env_paras["num_jobs"]),2),
                                            str.zfill(str(env_paras["num_mas"]),2))
