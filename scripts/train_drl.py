...
from ipps_drl.generator.case_generator_ipps import CaseGenerator
...
def main():
    ...
    is_validate = os.path.exists("data/dev/{0}{1}/".format(str.zfill(str(env_paras["num_jobs"]),2),
    ...
    writer_100._save()
    writer_100.close()
    # addcase = CaseGenerator(1, num_mas, job_folder='data/jobs/job_with_mas_5/')
    ...
    for i in tqdm.tqdm(range(1, train_paras["max_iterations"]+1), desc="Training"):
        ...
        if (i - 1) % train_paras["parallel_iter"] == 0:
            ...
            case = CaseGenerator(num_jobs, num_mas, job_folder=f'data/jobs/job_with_mas_{num_mas}/')
