...
from ipps_drl.env.load_data import nums_detec, load_ipps
# from ipps_drl.env.load_fjsp import nums_detec_fjsp  # FJSP loader not in repo
from ipps_drl.utils.trick import shrink_schedule
from ipps_drl.utils.sol_convert import sort_sol
from ipps_drl.inference.mcts import mcts  # tree_parallel/leaf_parallel removed in cleanup
...
from ipps_drl.utils.draw_gantt import draw_gantt
...
def main():
    ...
    for d_path in ["0605"]:
        ...
        # data_path = "data/test/{0}/problem/".format(test_paras["data_path"])
        # solution_path = "data/test/{0}/drl_solution/".format(test_paras["data_path"])
        # lb_path = "data/test/{0}/solution/".format(test_paras["data_path"])
        data_path = "data/test/{0}/problem/".format(test_paras["data_path"])
        solution_path = "data/test/{0}/drl_solution/".format(test_paras["data_path"])
        lb_path = "data/test/{0}/solution/".format(test_paras["data_path"])
        ...
        mod_files = os.listdir('checkpoints/')

        # data_path = './'
        # test_files = ["data/test/dumb/problem/dumb.txt"]
        ...
        if "DRL" in rules:
            for root, ds, fs in os.walk('checkpoints/'):
                for f in fs:
                    if f.endswith('.pt'):
                        rules.append(f)
        if len(rules) != 1:
        ...
        for i_rules in range(len(rules)):
            ...
            if rule.endswith('.pt'):
                if use_cuda:
                    model_CKPT = torch.load('checkpoints/' + mod_files[i_rules], map_location = device)
                else:
                    model_CKPT = torch.load('checkpoints/' + mod_files[i_rules], map_location = 'cpu')
            ...
            for i_ins in range(num_ins):
                ...
                ins_num_jobs, ins_num_mas, _ = nums_detec(lines)  # FJSP path disabled (load_fjsp module missing)
                if has_lb:
