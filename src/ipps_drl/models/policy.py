from ipps_drl.network.models import GraphEmbedding, Actor, Critic
from ipps_drl.network.utils import *
import torch
import torch.nn.functional as F
from torch.distributions import Categorical
import copy
from ipps_drl.models.memory import MemoryRL, MemoryIL
from ipps_drl.env.ipps_env import EnvState
class Policy(torch.nn.Module):
    ...
    def embs_to_probs(self, embs_local_global, ptr_proc, embs_wait, ptr_wait, indicator_wait, log_probs=False, return_max_wait_idxes=False, temperature=1.0):
        """
        ...
        Args:
            ...
            temperature (float, optional): Softmax temperature applied to the outer action distribution.
                ``> 1`` flattens, ``< 1`` sharpens, ``1`` keeps the original behaviour. Default ``1.0``.
        ...
        if self.qkv:
            ...
            if can_wait & torch.any(indicator_wait):
                ...
                action_probs_all = F.softmax(actor_all / temperature, dim=-1) if not log_probs else F.log_softmax(actor_all / temperature, dim=-1)
                ...
                actor_proc = shift_to_batch(proc_nodes, ptr_proc)
                if log_probs:
                    return F.log_softmax(actor_proc / temperature, dim=-1)
                action_probs = F.softmax(actor_proc / temperature, dim=-1)
        else:
...
class DRLPolicy(Policy):
    ...
    def act(
        ...
        with torch.no_grad():
            ...
            if flag_train:
                ...
                from ipps_drl.network.hetero_data import snapshot_batch as _snap
    ...
    def action_with_prob(self, state: EnvState, temperature: float = 1.0):
        """Return the policy distribution over eligible (ope, mas) pairs without sampling.

        Used by tree-search methods (e.g. MCTS) that need raw priors rather than a
        sampled action. The returned distribution is tempered by ``temperature``
        (``> 1`` flattens, ``< 1`` sharpens, ``1`` matches the trained policy exactly).

        Args:
            state: Current :class:`EnvState`.
            temperature: Softmax temperature applied to the outer action distribution.

        Returns:
            action_probs: Tensor of shape ``(n_active, max_eligible_per_batch + 1)``
                with one wait slot appended.
            ope_mas_job_pair_list: One ``(3, num_eligible_i)`` long-tensor per batch
                row, columns ``[ope, mas, job]`` aligned with the leading
                ``num_eligible_i`` entries of ``action_probs[i]``.
        """
        with torch.no_grad():
            batch_idxes = state.batch_idxes
            eligible = state.eligible_pairs
            ...
            _, embs_local_global, ptr_proc, embs_wait, ptr_wait, indicator_wait = \
                self.get_embedding(
                    state.graph, eligible, state.future_eligible_pairs,
                    state.opes_appertain_batch.long(), batch_idxes,
                )

            if self.wait_deal == 'max':
                action_probs, _ = self.embs_to_probs(
                    embs_local_global, ptr_proc, embs_wait, ptr_wait,
                    indicator_wait=indicator_wait,
                    return_max_wait_idxes=True,
                    temperature=temperature,
                )
            elif self.wait_deal in ('mean', 'softmax'):
                action_probs = self.embs_to_probs(
                    embs_local_global, ptr_proc, embs_wait, ptr_wait,
                    indicator_wait=indicator_wait,
                    temperature=temperature,
                )
            elif self.wait_deal == 'flexible':
                # The 'flexible' branch in embs_to_probs additionally consumes
                # eligible/future_eligible; passed through for completeness even
                # though embs_to_probs would need to be extended to accept them.
                action_probs = self.embs_to_probs(
                    embs_local_global, ptr_proc, embs_wait, ptr_wait,
                    indicator_wait=indicator_wait,
                    eligible=eligible, future_eligible=state.future_eligible_pairs,
                    temperature=temperature,
                )
            else:
                raise ValueError(f"Unsupported wait_deal: {self.wait_deal!r}")

            if torch.isnan(action_probs).any():
                raise ValueError("Nan in action_probs!")

            ope_mas_job_pair_list = []
            for idx in state.batch_idxes:
                eligible_pair = state.eligible_pairs[idx].nonzero().t()
                job_idx = state.opes_appertain_batch[idx, eligible_pair[0]]
                ope_mas_job_pair = torch.cat([eligible_pair, job_idx.unsqueeze(0)], dim=0)
                ope_mas_job_pair_list.append(ope_mas_job_pair)

            return action_probs, ope_mas_job_pair_list
